...
start_fixtures() {
  ...
  # `>>`, not `>`, on both branches: with_fixtures_stopped restarts this httpd
  # mid-run (scenario 3), and truncating there silently invalidated every count
  # baseline taken before the restart -- scenario 3's own
  # RELOAD_GETS_BEFORE_RECOVERY was captured while the httpd was down and then
  # compared against a log that had restarted at zero. Appending keeps the log
  # monotonic for the whole run, which is what the per-scenario baselines assume.
  if [ -n "${KIOSK_HTTPD_BIN:-}" ]; then
    ...
    # Python's line-buffering default) -- every httpd_get_count check would read a
    # truncated/empty log until the buffer happened to fill or the process exited.
    #
    # `Cache-Control: no-store`, matching fixture-httpd (kiosk-smoke/src/httpd.rs),
    # which is why this is a handler subclass and not a bare `-m http.server`.
    # Every scenario treats the access log as a faithful record of top-level
    # navigations; without no-store that is FALSE -- WebKit serves a re-navigation
    # to an already-loaded URL out of its own cache and the httpd never hears about
    # it. That is not hypothetical: it made scenario 4's "fresh GET after the crash"
    # report a recovery failure while recovery had in fact worked (proven
    # independently -- the reloaded page's own JS re-ran and produced a nav.blocked).
    # The two servers MUST agree here, or the harness proves different things under
    # CI than it does for a human running it locally.
