...
fn serve(root: &Path, mut stream: TcpStream, requests: &Arc<Mutex<Vec<String>>>) -> io::Result<()> {
    ...
    let disposition = disposition
        ...
        .unwrap_or_default();
    // `Cache-Control: no-store` on every response, matching the shell driver's
    // python fallback. The smoke scenarios treat this server's access log as a
    // faithful record of top-level navigations, and without it that is false:
    // WebKit serves a re-navigation to an already-loaded URL out of its own cache
    // and never asks us again. Scenario 4's post-crash "recovery navigated home"
    // check reads as a failure in exactly that case, while recovery in fact worked.
    // The two servers must agree here or CI proves something different from a
    // local run.
    write!(
        stream,
